...
* `--latest false`: (optional) Set to false to download the minimum required version of all dependencies unless some specific version is explicitly requested. (default: true)
* `--latest-specified`: (optional) (advanced) Set to true to download latest dependencies of any plugin that is requested to have the latest version unless some specific version is explicitly requested. All other plugin dependency versions are determined by the update center metadata or the plugin MANIFEST.MF.
